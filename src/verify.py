...
# Set to store bonds that have occurred
bonds = set()
...
for index, line in enumerate(log):
    ...
    # Check if bond has occurred twice
    current_bond = (token1, token2, token3)
    if current_bond in bonds:
        print(f"Duplicate bond confirmation received at line {index + 1}")
        sys.exit()
    else:
        bonds.add(current_bond)
