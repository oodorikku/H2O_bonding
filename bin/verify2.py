...
def parse_timestamp(timestamp_str):
    return datetime.strptime(timestamp_str, "%a %b %d %H:%M:%S PST %Y")
...
def check_all_bonds_have_requested_molecules(server_log):
    ...
    def to_datetime(timestamp):
        return datetime.strptime(timestamp, "%a %b %d %H:%M:%S PST %Y")
...
h_log = read_log_file('./h_log.txt') # Change path name manually if it cant be found
o_log = read_log_file('./o_log.txt')
server_log = read_log_file('./server_log.txt')
